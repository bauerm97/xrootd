...
sub readNetworkInfo {
	...
	if(open(NET_DEV, "</proc/net/dev")){
		while (my $line = <NET_DEV>) {
			if($line =~ /\s*([ep]\w+\d):\s*(\d+)\s+\d+\s+(\d+)\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+\s+(\d+)\s+\d+\s+(\d+)/){
...
sub computeCummulativeParams {
	...
	for my $rawParam (keys %$dataRef){
		next if $rawParam !~ /^raw_eth/ && $rawParam !~ /^raw_em/ && $rawParam !~ /^raw_p4p/;
...
sub getFilteredData {
	...
	for my $param (@$paramsRef) {
		if($param eq "net_sockets"){
			...
			my $net_param = $1;
			for my $key (keys %$dataRef) {
				if ($key =~ /^e\w+\d_$net_param/ || $key =~ /^p4p\d_$net_param/){
